...
const App = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Initialize counters for KNN and RF second predictions
      let knnCount = 0;
      let rfCount = 0;
      ...
      Object.entries(response.data).forEach(([key, value]) => {
        if (key.includes('Prediction')) {
          ...
          const modelBase = isKNN ? 'KNN' : 'RF';
          // Determine if it's a second prediction
          const model = (isKNN ? ++knnCount : ++rfCount) > 1 ? `${modelBase} 2nd Prediction` : modelBase;
